    ...
    <div class="mx-auto max-w-6xl px-6 pb-20 pt-20 sm:pb-40">
      ...
      >
        <div class="max-w-sm">
          ...
          <p class="mt-5 text-lg leading-relaxed text-white/80">
            {{
              t(
                "footer_tagline",
                "Baked earth, shaped by hand — workshops and pieces from our studio.",
                "طين مشوي يُشكّل باليد — ورشات وقطع من الاستوديو.",
              )
            }}
          </p>
        </div>
        ...
        <div class="grid flex-1 gap-10 sm:grid-cols-3 lg:max-w-2xl">
          ...
          <div
            v-if="social.length || storeBadges.length"
      ...
      <div
        v-if="registration.length"
        class="mt-16 flex flex-wrap gap-3 border-t border-white/15 pt-8"
      >
        <component
          :is="entry.url ? 'a' : 'div'"
        ...
        >
          ...
          <span class="flex min-w-0 flex-col gap-0.5">
            <span class="text-[11px] uppercase leading-none text-white/50">{{
              entry.label
            }}</span>
            <span
              class="text-sm font-semibold leading-tight text-white"
              dir="ltr"
              >{{ entry.value }}</span
            >
          </span>
      ...
      <div
        class="mt-10 flex flex-col gap-3 border-t border-white/15 pt-6 text-sm text-white/60 sm:flex-row sm:items-center sm:justify-between"
      >
        ...
        <p>
          {{ t("footer_made", "Handmade in Amman", "مصنوع يدويًا في عمّان") }}
        </p>
      </div>
    </div>
  </footer>
...
<script setup>
const { social, contact, appStore, googlePlay, appGallery, business } =
  useAppSettings();
