<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 py-10" role="dialog" aria-modal="true">
      ...
      <div class="relative w-full max-w-lg rounded-2xl border bg-background p-6 shadow-lg">
        ...
        <div
          ...
          class="prose prose-sm max-h-[60svh] max-w-none overflow-y-auto dark:prose-invert [&_a]:text-brand-rust [&_a]:underline [&_h2]:mt-6 [&_h2]:mb-2 [&_h2]:text-xl [&_h2]:font-semibold [&_h3]:mt-4 [&_h3]:mb-2 [&_h3]:font-semibold [&_li]:my-1 [&_p]:my-3 [&_ul]:list-disc [&_ul]:ps-6"
          v-html="page.content"
        />

        <Button class="mt-6 h-12 w-full rounded-xl bg-brand-rust text-base hover:bg-brand-rust/90" @click="emit('update:open', false)">
          {{ t('close', 'Close', 'إغلاق') }}
        </Button>
      </div>
    </div>
