<template>
  <ul class="flex flex-col divide-y">
    <li v-for="item in items" :key="item.id" class="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
      <component
        :is="item.product ? NuxtLink : 'span'"
        :to="item.product ? `/shop/${item.product.id}` : undefined"
        class="block size-16 shrink-0 overflow-hidden rounded-xl border bg-brand-mist"
      >
      ...
      <span class="shrink-0 font-display text-base font-semibold">{{ format(item.line_total) }}</span>
    </li>
  </ul>
</template>
