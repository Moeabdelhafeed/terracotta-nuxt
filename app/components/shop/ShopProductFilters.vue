  ...
  <div class="mb-10 flex flex-col gap-4">
    <div class="flex flex-wrap items-center gap-3">
      ...
      <select
        ...
        :aria-label="t('sort_by', 'Sort by', 'الترتيب')"
      ...
      >
        <option value="">{{ t('sort_curated', 'Studio order', 'ترتيب الاستوديو') }}</option>
        <option v-for="option in SORTS" :key="option" :value="option">{{ sortLabel(option) }}</option>
      </select>
    ...
    <div class="flex flex-wrap items-center gap-3">
      <div class="flex h-12 items-center gap-2 rounded-xl border border-input px-3">
        <LucideTag class="size-4 shrink-0 text-muted-foreground" />
        <span class="text-sm text-muted-foreground">{{ t('price_range', 'Price', 'السعر') }}</span>
        <Input
          v-model="minInput"
          data-test="price-min"
          type="number"
      ...
      <span v-if="priceInverted" data-test="price-hint" class="text-xs text-destructive">
        {{ t('price_range_invalid', 'The highest price has to be above the lowest.', 'يجب أن يكون السعر الأعلى أكبر من الأدنى.') }}
      </span>
    </div>

    <div v-if="chips.length" class="flex flex-wrap items-center gap-2" data-test="active-filters">
      <span class="text-xs text-muted-foreground">{{ t('active_filters', 'Filtering by', 'التصفية حسب') }}</span>
      ...
      <Button variant="ghost" size="sm" class="rounded-full text-muted-foreground" @click="emit('clear')">
...
export const productApiQuery = (query = {}, perPage = PRODUCTS_PER_PAGE) => {
  ...
  const all = {
    ...
    min_price: min,
    max_price: max !== undefined && (min === undefined || max >= min) ? max : undefined,
...
watch(() => props.query, (query) => {
  ...
  if ((query.min_price ?? '') !== minInput.value) minInput.value = query.min_price ?? ''
  if ((query.max_price ?? '') !== maxInput.value) maxInput.value = query.max_price ?? ''
})
...
watchDebounced(term, (value) => emit('apply', { search: value || null }), { debounce: 400 })
...
const categoryTitle = (id) => categories.value.find((category) => category.id === id)?.title ?? ''

const subCategoryTitle = (id) => categories.value
...
const priceChipLabel = () => {
  ...
  if (min !== undefined) return t('price_from', 'From :amount', 'من :amount', { amount: format(min) })
  return t('price_up_to', 'Up to :amount', 'حتى :amount', { amount: format(max) })
}
...
const chips = computed(() => {
  ...
  if (applied.category_id) {
    list.push({ key: 'category', label: categoryTitle(applied.category_id), patch: { category: null, sub: null } })
  }
  if (applied.sub_category_id) {
    list.push({ key: 'sub', label: subCategoryTitle(applied.sub_category_id), patch: { sub: null } })
  }
  if (applied.search) {
    list.push({ key: 'search', label: `“${applied.search}”`, patch: { search: null } })
  }
  if (applied.featured) {
    list.push({ key: 'featured', label: t('featured', 'Featured', 'مميز'), patch: { featured: null } })
  }
  if (applied.on_sale) {
    list.push({ key: 'sale', label: t('on_sale', 'On sale', 'خصم'), patch: { sale: null } })
  }
  if (applied.min_price !== undefined || applied.max_price !== undefined) {
    list.push({ key: 'price', label: priceChipLabel(), patch: { min_price: null, max_price: null } })
  }
  if (applied.sort) {
    list.push({ key: 'sort', label: sortLabel(applied.sort), patch: { sort: null } })
  }
