<template>
  <div class="rounded-2xl border bg-card p-5">
    ...
    <div v-if="state === 'settled'" class="flex items-start gap-3">
      <span class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-brand-green/10 text-brand-green">
        <LucideCheckCircle2 class="size-5" />
      </span>
      <div>
        <p class="font-display text-base font-semibold text-foreground">{{ t('hold_settled_title', 'All paid', 'تم الدفع بالكامل') }}</p>
        <p class="mt-1 text-sm text-muted-foreground">{{ t('hold_settled_body', 'Nothing left to pay — you are all set.', 'لا يوجد مبلغ متبقٍ — كل شيء جاهز.') }}</p>
      </div>
    </div>
    ...
    <div v-else-if="state === 'expired'" class="flex flex-col gap-4">
      <div class="flex items-start gap-3">
        <span class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-destructive/10 text-destructive">
          <LucideTimerOff class="size-5" />
        </span>
        <div>
          <p class="font-display text-base font-semibold text-foreground">{{ t('hold_expired_title', 'Payment window closed', 'انتهت مهلة الدفع') }}</p>
          <p class="mt-1 text-sm text-muted-foreground">{{ expiredMessage || t('hold_expired_body', 'This payment window has closed. Please start again.', 'انتهت مهلة الدفع. يرجى البدء من جديد.') }}</p>
        </div>
      </div>
      <Button v-if="restartTo" as-child class="h-12 rounded-xl bg-brand-rust text-base hover:bg-brand-rust/90">
        <NuxtLink :to="restartTo">{{ restartLabel || t('start_again', 'Start again', 'البدء من جديد') }}</NuxtLink>
      </Button>
    </div>
    ...
    <div v-else class="flex flex-col gap-4">
      <div class="flex items-center justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-foreground">{{ t('hold_reserved', 'Reserved for you', 'محجوز لك') }}</p>
            <p class="text-xs text-muted-foreground">{{ t('hold_pay_within', 'Complete payment before the timer runs out.', 'أكمل الدفع قبل انتهاء الوقت.') }}</p>
          </div>
        </div>
      ...
      <Button
        ...
        @click="onPay"
      >
        {{ paying ? t('paying', 'Paying...', 'جارٍ الدفع...') : t('pay_now', 'Pay now', 'ادفع الآن') }}
      </Button>
    </div>
  </div>
</template>
...
const expiresMs = computed(() => (props.expiresAt ? new Date(props.expiresAt).getTime() : NaN))
const remaining = computed(() => (Number.isNaN(expiresMs.value) ? 0 : Math.max(0, Math.floor((expiresMs.value - now.value) / 1000))))

const state = computed(() => {
  if (props.paymentStatus === 'paid' || isZeroMoney(props.amountDue)) return 'settled'
...
const onPay = async () => {
  ...
  try {
    ...
    const holdMessage = holdKeys.map((key) => fieldError(normalized, key)).find(Boolean)
    if (normalized.status === 422 && holdMessage) {
      ...
      errorText.value = normalized.message || t('payment_failed', 'Payment failed. Please try again.', 'فشل الدفع. حاول مرة أخرى.')
