<template>
  <section class="rounded-3xl border bg-card p-6" data-test="upload">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div class="min-w-0">
        <h2 class="font-display text-xl font-semibold">{{ t('upload_title', 'Photograph your pieces', 'صوّري قطعك') }}</h2>
        <p class="mt-1 text-sm text-muted-foreground">
          {{ t('upload_note', 'Add a piece for each object you made, then attach its photos — every angle of the same object belongs to the same piece.', 'أضيفي قطعة لكل شيء صنعتِه، ثم أرفقي صورها — كل زوايا الشيء نفسه تخص القطعة نفسها.') }}
        </p>
      </div>
      <span class="shrink-0 rounded-full bg-brand-mist/60 px-3 py-1 text-xs font-medium tabular-nums" data-test="piece-progress">
        {{ t('pieces_progress', ':done of :expected named', ':done من :expected مسماة', { done: savedPieces.length, expected }) }}
      </span>
    </div>

    <p class="mt-4 rounded-2xl bg-amber-100/70 p-4 text-sm text-amber-900">
      {{ t('upload_window', 'Photos can only be added while the session is running. Once the studio finishes it nothing can ever be added to this booking again.', 'يمكن إضافة الصور أثناء الورشة فقط. بمجرد أن ينهيها الاستوديو لن يمكن إضافة أي شيء إلى هذا الحجز مرة أخرى.') }}
    </p>

    <ul class="mt-6 flex flex-col gap-4">
      <li v-for="piece in savedPieces" :key="`saved-${piece.id}`" class="rounded-2xl border p-4" data-test="saved-piece">
        ...
        <ul v-if="additions[piece.id]?.length" class="mt-2 flex flex-col gap-1">
          <li v-for="(file, index) in additions[piece.id]" :key="index" class="flex items-center gap-2 text-xs text-muted-foreground">
      ...
      <li v-for="draft in drafts" :key="draft.key" class="rounded-2xl border border-dashed p-4" data-test="draft-piece">
        ...
        <input
          ...
          :aria-label="t('piece_photos', 'Photos of this piece', 'صور هذه القطعة')"
          @change="pick($event, { draft })"
        >

        <ul v-if="draft.files.length" class="mt-2 flex flex-col gap-1">
          <li v-for="(file, index) in draft.files" :key="index" class="flex items-center gap-2 text-xs text-muted-foreground">
            <LucideImage class="size-3.5 shrink-0" />
            <span class="min-w-0 flex-1 truncate">{{ file.name }}</span>
          </li>
        </ul>
    ...
    <p v-if="atCeiling" class="mt-3 text-xs text-muted-foreground" data-test="ceiling-note">
      {{ t('pieces_ceiling', 'You bought :n piece(s), so that is everything this booking can hold.', 'اشتريتِ :n قطعة، وهذا كل ما يمكن أن يحتويه هذا الحجز.', { n: expected }) }}
    </p>
    <p v-else-if="!hasCeiling && savedPieces.length >= expected" class="mt-3 text-xs text-muted-foreground" data-test="guide-note">
      {{ t('pieces_guide', 'That is everything we expected — add more if you made more.', 'هذا كل ما توقعناه — أضيفي المزيد إن صنعتِ أكثر.') }}
    </p>
    ...
    <span v-if="fieldError(errors, 'piece_labels')" class="mt-1 block text-xs text-destructive">{{ fieldError(errors, 'piece_labels') }}</span>
    <span v-if="fieldError(errors, 'piece_keys')" class="mt-1 block text-xs text-destructive">{{ fieldError(errors, 'piece_keys') }}</span>
    <span v-if="fieldError(errors, 'piece_ids')" class="mt-1 block text-xs text-destructive">{{ fieldError(errors, 'piece_ids') }}</span>
    <span v-if="fieldError(errors, 'images')" class="mt-1 block text-xs text-destructive">{{ fieldError(errors, 'images') }}</span>
...
const atCeiling = computed(() => hasCeiling.value && savedPieces.value.length + drafts.value.length >= expected.value)

const staged = computed(() => [
  ...savedPieces.value.flatMap((piece) => (additions.value[piece.id] ?? []).map((file) => ({ file, label: piece.label, id: piece.id }))),
  ...drafts.value.flatMap((draft) => draft.files.map((file) => ({ file, label: draft.label.trim(), key: draft.key }))),
])

const budget = computed(() => Math.max(0, MAX_PHOTOS_PER_PERSON * (props.booking.people_count ?? 1) - (props.booking.images?.length ?? 0)))
const photosLeft = computed(() => Math.max(0, budget.value - staged.value.length))

const canUpload = computed(() => !!staged.value.length && !uploading.value
  && drafts.value.every((draft) => !draft.files.length || draft.label.trim()))

const addDraft = () => {
  if (atCeiling.value) return
  drafts.value = [...drafts.value, { key: String(++sequence), label: '', files: [] }]
}

const removeDraft = (draft) => { drafts.value = drafts.value.filter((candidate) => candidate.key !== draft.key) }

const pick = (event, target) => {
  const held = target.draft ? target.draft.files.length : (additions.value[target.pieceId] ?? []).length
