      ...
      <div class="flex flex-col gap-5">
        <div>
          <h1 class="font-display text-3xl font-semibold sm:text-4xl">
            {{ t("complaints_title", "Complaints", "الشكاوى") }}
          </h1>
          <p class="mt-3 text-muted-foreground">
            {{
              t(
                ...
                "أخبرنا بما حدث وسنعود إليك في أقرب وقت.",
              )
            }}
          </p>
        </div>
        ...
        <form
          class="grid gap-4 rounded-3xl border bg-card p-6 sm:grid-cols-2 sm:p-8"
        ...
        >
          ...
          <template v-if="!isRegistered">
            <div class="grid gap-2">
              ...
              <Input
                id="complaint_name"
                v-model="form.name"
                data-test="complaint-name"
                type="text"
                maxlength="120"
                class="h-12 rounded-xl text-base"
                required
              />
              <span v-if="err('name')" class="text-xs text-destructive">{{
                err("name")
              }}</span>
            </div>
            <div class="grid gap-2">
              <Label for="complaint_contact">{{
                t(
                  "complaint_contact",
                  "Email or phone we can reply to",
                  "بريد أو هاتف للتواصل معك",
                )
              }}</Label>
              <Input
                id="complaint_contact"
                v-model="form.contact"
                data-test="complaint-contact"
                type="text"
                maxlength="120"
                class="h-12 rounded-xl text-base"
                dir="ltr"
                required
              />
              <span v-if="err('contact')" class="text-xs text-destructive">{{
                err("contact")
              }}</span>
            </div>
          </template>
          ...
          <div class="grid gap-2">
            ...
            <Input
              ...
              dir="ltr"
            />
            <span v-if="err('reference')" class="text-xs text-destructive">{{
              err("reference")
            }}</span>
          </div>
          ...
          <div class="grid gap-2 sm:col-span-2">
            <Label for="complaint_message">{{
              t("complaint_message", "What happened?", "ماذا حدث؟")
            }}</Label>
            <textarea
              ...
              :placeholder="
                t(
                  'complaint_message_placeholder',
                  'Describe the problem in a few lines…',
                  'اشرح المشكلة في بضعة أسطر…',
                )
              "
            />
            <div class="flex flex-wrap items-center justify-between gap-x-3">
              ...
              <span v-else class="text-xs text-muted-foreground">{{
                t(
                  "complaint_message_hint",
                  "At least 5 characters, no HTML.",
                  "5 أحرف على الأقل، بدون وسوم HTML.",
                )
              }}</span>
          ...
          <span
            v-if="submitError"
            data-test="complaint-error"
            class="text-xs text-destructive sm:col-span-2"
            >{{ submitError }}</span
          >
          <p
            v-if="sentMessage"
            data-test="complaint-success"
            class="rounded-2xl bg-brand-mist/60 p-4 text-sm text-foreground sm:col-span-2"
          >
            {{ sentMessage }}
          </p>
          ...
          >
            {{
              submitting
                ...
                : t("complaint_send", "Send complaint", "إرسال الشكوى")
            }}
          </Button>
        </form>
            ...
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                <span class="text-sm font-medium text-foreground">{{
                  complaintTypeLabel(complaint.type, t)
                }}</span>
                <span
              ...
              <p
                class="mt-2 break-words whitespace-pre-line text-sm text-muted-foreground"
              >
                {{ complaint.message }}
              </p>
              <p
                v-if="complaint.reference"
              ...
              <p class="mt-2 text-xs text-muted-foreground">
                ...
                <template v-if="complaint.resolved_at">
                  ...
                  {{
                    t(
                      "complaint_resolved_on",
                      "Resolved :date",
                      "تم الحل :date",
                      { date: formatDate(complaint.resolved_at) },
                    )
                  }}
                </template>
              </p>
