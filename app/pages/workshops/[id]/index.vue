    ...
    <div class="grid gap-10 lg:grid-cols-[1.4fr_1fr] lg:items-start">
      ...
      <div class="rounded-3xl border bg-card p-6">
        <AppSkeleton
          v-for="n in 4"
          :key="n"
          class="mb-4 h-16 !rounded-2xl last:mb-0"
        />
      </div>
    </div>
  </main>
    ...
    <div class="mx-auto max-w-6xl px-6 py-16">
      <div class="grid gap-10 lg:grid-cols-[1.4fr_1fr] lg:items-start">
        <div ref="content">
          ...
          <section v-if="workshop.categories?.length" class="mt-12">
            <h2 class="font-display text-2xl font-semibold">
              {{
                t(
                  "workshop_catalogue",
                  "Pieces you can choose from",
                  "قطع يمكنك الاختيار منها",
                )
              }}
            </h2>
            <p v-if="perPersonNote" class="mt-2 text-sm text-muted-foreground">
              {{ perPersonNote }}
            </p>
          ...
          <section
            v-if="ownPieces"
            class="mt-12 rounded-3xl border bg-brand-mist/40 p-6"
          >
            <h2 class="font-display text-2xl font-semibold">
              {{
                t(
            ...
            <p class="mt-2 text-sm text-muted-foreground">
              {{
                Number(ownPieces.price) > 0
                  ? t(
                      "own_pieces_price",
                      "Bring back a piece from an earlier session — :price to paint it.",
                      "أحضر قطعة من جلسة سابقة — :price لتلوينها.",
                      { price: format(ownPieces.price) },
                    )
                  : t(
                      "own_pieces_free",
                      "Bring back a piece from an earlier session and paint it at no extra cost.",
                      "أحضر قطعة من جلسة سابقة ولوّنها دون تكلفة إضافية.",
                    )
              }}
            </p>
            ...
            <p v-else class="mt-4 text-sm text-muted-foreground">
              {{
                t(
                  "own_pieces_empty",
                  "Pieces from your finished sessions show up here.",
                  "ستظهر هنا القطع من جلساتك المكتملة.",
                )
              }}
            </p>
...
const perPersonNote = computed(() => {
  ...
  if (min && max && min !== max) {
    return t(
      "pieces_range",
      ":min to :max pieces per person.",
      "من :min إلى :max قطعة للشخص.",
      { min, max },
    );
  }

  return t("pieces_count", ":n pieces per person.", ":n قطعة للشخص.", {
    n: max ?? min,
  });
});
...
useSchemaOrg([
  defineProduct({
    ...
    offers: () =>
      Number(workshop.value?.price) > 0
        ? [
            {
              price: Number(workshop.value.price),
              priceCurrency: "SAR",
