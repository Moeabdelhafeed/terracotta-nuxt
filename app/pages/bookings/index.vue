  ...
  <main class="min-h-svh bg-background pb-28">
    ...
    <div class="mx-auto max-w-6xl px-6 py-16">
      ...
      <div class="mt-8 flex flex-wrap items-center gap-3">
        <ul class="-mx-6 flex flex-1 gap-2 overflow-x-auto px-6 pb-1" data-test="status-tabs">
          <li v-for="tab in tabs" :key="tab.key" class="shrink-0">
            <Button
              size="sm"
              class="rounded-xl"
              :variant="tab.key === activeTab ? 'default' : 'outline'"
              :class="tab.key === activeTab ? 'bg-brand-rust hover:bg-brand-rust/90' : ''"
              :data-status="tab.key"
        ...
        <label class="sr-only" for="bookings-sort">{{ t('sort_by', 'Sort by', 'الترتيب') }}</label>
        <select
          id="bookings-sort"
          ...
          :value="activeSort"
          @change="apply({ sort: $event.target.value === 'newest' ? null : $event.target.value })"
        >
          <option v-for="option in BOOKING_SORTS" :key="option" :value="option">{{ sortLabel(option) }}</option>
        </select>
      </div>

      <div v-if="pending && !items.length" class="mt-8 grid gap-4 lg:grid-cols-2" aria-busy="true">
        <AppSkeleton v-for="n in 4" :key="n" class="h-32 w-full !rounded-3xl" />
      </div>

      <p v-else-if="!items.length" class="mx-auto mt-8 max-w-xl rounded-3xl border border-dashed p-10 text-center text-muted-foreground">
        {{ activeTab === 'all'
          ? t('bookings_empty', 'You have no bookings yet.', 'لا توجد لديك حجوزات بعد.')
          : t('bookings_empty_filtered', 'No bookings match this filter.', 'لا توجد حجوزات تطابق هذا التصنيف.') }}
      </p>
...
const activeTab = computed(() => (BOOKING_TABS.includes(route.query.status) ? route.query.status : 'all'))
const activeSort = computed(() => (BOOKING_SORTS.includes(route.query.sort) ? route.query.sort : 'newest'))

const { items, lastPage, pending, statusCounts } = useBookings({
  page,
  perPage: 10,
  status: computed(() => (activeTab.value === 'all' ? undefined : activeTab.value)),
  sort: computed(() => (activeSort.value === 'newest' ? undefined : activeSort.value)),
})
...
const tabs = computed(() => {
  ...
  return BOOKING_TABS
    .filter((key) => key === 'all' || key === activeTab.value || (counts?.[key] ?? 0) > 0)
    .map((key) => ({ key, label: tabLabels.value[key] ?? key, count: counts?.[key] ?? null }))
})

const sortLabel = (option) => ({
  newest: t('sort_newest', 'Newest booked', 'الأحدث حجزًا'),
  oldest: t('sort_oldest', 'Oldest booked', 'الأقدم حجزًا'),
  session_soonest: t('sort_session_soonest', 'Nearest session', 'الجلسة الأقرب'),
  session_latest: t('sort_session_latest', 'Furthest session', 'الجلسة الأبعد'),
}[option] ?? option)
