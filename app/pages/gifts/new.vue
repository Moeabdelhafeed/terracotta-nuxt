    ...
    <div class="mx-auto max-w-6xl px-6 py-16">
      ...
      <section v-if="unavailable" class="mx-auto max-w-xl rounded-3xl border bg-card p-6 text-center sm:p-8">
        ...
        <h1 class="mt-5 font-display text-2xl font-semibold">
          {{ t('gift_off_title', 'Gift credit is unavailable', 'إهداء الرصيد غير متاح حاليًا') }}
        </h1>
        <p class="mt-3 text-sm text-muted-foreground">
          {{ unavailableMessage || t('gift_off_body', 'Gifting is switched off right now. Please check back later.', 'خدمة الإهداء متوقفة حاليًا. يرجى المحاولة لاحقًا.') }}
        </p>
      ...
      <template v-else>
        <header class="mx-auto max-w-xl text-center">
          ...
          <h1 class="mt-3 font-display text-3xl font-semibold sm:text-4xl">
            <span v-if="packageAmount">{{ t('gift_new_title', 'Gift :amount of credit', 'اهداء رصيد :amount', { amount: format(packageAmount) }) }}</span>
            <AppSkeleton v-else class="mx-auto h-9 w-64" />
          </h1>
          <p class="mt-3 text-muted-foreground">
            {{ t('gift_new_subtitle', 'Send a credit gift to someone you love, to enjoy our pottery and ceramics workshops.', 'أرسل هدية رصيد لأحبائك للاستمتاع بورشات الفخار والسيراميك') }}
          </p>
        </header>
        ...
        <div v-if="gift" class="mx-auto mt-10 grid max-w-4xl gap-6 lg:grid-cols-[1fr_22rem] lg:items-start">
          <section class="rounded-3xl border bg-card p-6 sm:p-8">
            <h2 class="font-display text-2xl font-semibold">{{ t('gift_pay_title', 'Payment', 'الدفع') }}</h2>
            <p class="mt-2 text-sm text-muted-foreground">
              {{ t('gift_pay_body', 'Confirm the gift and pay to get your share link.', 'أكّد الهدية وادفع للحصول على رابط المشاركة.') }}
            </p>

            <dl class="mt-6 flex flex-col gap-2 rounded-2xl bg-brand-mist/40 p-4 text-sm">
        ...
        <form v-else class="mx-auto mt-10 grid max-w-4xl gap-6 lg:grid-cols-[1fr_22rem] lg:items-start" @submit.prevent="purchase">
          <section class="flex flex-col gap-5 rounded-3xl border bg-card p-6 sm:p-8">
            <div class="flex flex-col gap-2">
              <Label for="recipient_name">{{ t('gift_recipient_name', "Recipient's name", 'اسم المهدى له') }}</Label>
              <Input
                id="recipient_name"
                ...
                :placeholder="t('gift_recipient_name_ph', 'e.g. Sara', 'مثال: سارة')"
              />
              <span v-if="fieldError(errors, 'recipient_name')" class="text-xs text-destructive">{{ fieldError(errors, 'recipient_name') }}</span>
            </div>

            <div class="flex flex-col gap-2">
              <Label for="recipient_phone">
                {{ t('gift_recipient_phone', 'Phone', 'رقم الجوال') }}
                <span class="text-xs font-normal text-muted-foreground">({{ t('optional', 'optional', 'اختياري') }})</span>
              </Label>
              <AuthPhoneInput id="recipient_phone" v-model="form.recipient_phone" :allowed="allowedPhoneCountries" />
              <p class="text-xs text-muted-foreground">
                {{ t('gift_phone_hint', 'For your own record, and to open WhatsApp with the link ready. Nothing is sent automatically.', 'لسجلك الخاص، ولفتح واتساب والرابط جاهز. لا يُرسل شيء تلقائيًا.') }}
              </p>
              <span v-if="fieldError(errors, 'recipient_phone')" class="text-xs text-destructive">{{ fieldError(errors, 'recipient_phone') }}</span>
            </div>
            ...
            <div class="flex flex-col gap-2">
              ...
              <textarea
                ...
                :placeholder="t('gift_message_ph', 'Write your message here...', 'اكتب رسالتك هنا...')"
              />
              <div class="flex items-center justify-between gap-4">
                <span v-if="fieldError(errors, 'message')" class="text-xs text-destructive">{{ fieldError(errors, 'message') }}</span>
                <span class="ms-auto text-xs tabular-nums text-muted-foreground">{{ form.message.length }}/1000</span>
              </div>
            </div>

            <div class="h-px bg-border" />

            <CheckoutDiscountCodeInput v-model="discountCode" :errors="discountErrors" :disabled="busy" />
          ...
          <aside class="flex flex-col gap-4">
            ...
            <p v-if="giftValue" class="rounded-2xl bg-brand-green/10 px-4 py-3 text-sm text-brand-green">
              {{ t('gift_value_note', 'They receive :amount in wallet credit.', 'سيصله رصيد بقيمة :amount في المحفظة.', { amount: format(giftValue) }) }}
            </p>

            <span v-if="topError" class="text-sm text-destructive">{{ topError }}</span>
            ...
            >
              ...
              {{ creating ? t('please_wait', 'Please wait...', 'يرجى الانتظار...') : t('gift_pay_cta', 'Pay', 'الدفع') }}
            </Button>

            <Button as-child type="button" variant="outline" class="h-12 rounded-xl">
...
const { packageAmount, packageActive, packageStatus, quote: quoteGift, create, pay } = useGifts()
...
const { submit: submitCreate, pending: creating, errors, error: createError } = useSubmit()
...
const summaryQuote = computed(() => (quote.value ? { ...quote.value, delivery_fee: null } : null))
const giftValue = computed(() => quote.value?.gift_value ?? packageAmount.value)

const unavailableMessage = ref('')
const unavailable = computed(() => unavailableMessage.value !== '' || (packageStatus.value === 'success' && !packageActive.value))

const topError = computed(() => fieldError(errors.value, 'gift') || createError.value)
...
const runQuote = async () => {
  quoting.value = true
  try {
    const res = await quoteGift({ use_wallet: useWallet.value, discount_code: discountCode.value || null })
    quote.value = res?.data ?? null
    discountErrors.value = {}
  } catch (err) {
    const normalized = normalizeApiError(err)
    if (fieldError(normalized, 'gift')) unavailableMessage.value = normalized.message
...
watch(discountCode, (code) => { if (code) discountErrors.value = {} })

onMounted(runQuote)

const purchase = async () => {
  try {
    const { data } = await submitCreate(() => create({
...
useSeoMeta({ title: () => t('gift_new_short', 'New gift', 'هدية جديدة'), robots: 'noindex, nofollow' })
</script>
