      ...
      >
        ...
        <p class="mt-3 text-sm text-muted-foreground">
          {{
            t(
              "gift_missing_body",
              "This gift is not on your account.",
              "هذه الهدية ليست ضمن حسابك.",
            )
          }}
        </p>
        ...
        <header class="mx-auto max-w-xl text-center">
          ...
          <p class="mt-3 text-muted-foreground">
            <template v-if="cancelled">{{
              t(
                "gift_cancelled_body",
                "The payment window closed before it was paid, so the link no longer works.",
                "انتهت مهلة الدفع قبل إتمامه، لذلك لم يعد الرابط يعمل.",
              )
            }}</template>
            <template v-else-if="gift.is_redeemed">{{
              t(
            ...
            <template v-else>{{
              t(
                "gift_success_body",
                "Copy the link and send it to whoever the gift is for, so they can enjoy our Terracotta workshops.",
                "الرجاء نسخ الرابط و ارساله لمتلقي الهدية و الاستمتاع بورشات تيراكوتا",
              )
            }}</template>
          </p>
        </header>
        ...
        <div class="mt-8 grid gap-6 lg:grid-cols-[1fr_22rem] lg:items-start">
          <div class="flex min-w-0 flex-col gap-6">
            ...
            >
              ...
              <p
                v-else
                class="rounded-2xl bg-brand-green/10 px-4 py-4 text-center text-sm font-medium text-brand-green"
              >
                {{
                  t(
                    "gift_used_note",
                    "The credit is already in their wallet — the link is spent.",
                    "تم إضافة الرصيد إلى محفظتهم — الرابط مستخدم.",
                  )
                }}
              </p>
            </section>
            ...
            <dl
              class="flex flex-col gap-2 rounded-3xl border bg-card p-6 text-sm sm:p-8"
            >
              ...
              >
                <dt class="text-muted-foreground">
                  {{ t("gift_credit", "Credit", "الرصيد") }}
                </dt>
                <dd class="font-display text-lg font-semibold text-primary">
                  {{ format(gift.amount) }}
                </dd>
              </div>
            </dl>
...
const gift = computed(
  () =>
    items.value.find((row) => String(row.id) === String(route.params.id)) ??
    null,
);
