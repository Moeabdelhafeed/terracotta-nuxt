    ...
    <div class="mx-auto max-w-6xl px-6 py-16">
      <div class="flex flex-col gap-5">
        <h1 class="font-display text-3xl font-semibold sm:text-4xl">
          {{ t('orders_title', 'My orders', 'طلباتي') }}
        </h1>
        ...
        <p v-if="total" class="mt-6 text-center text-sm text-muted-foreground">
          {{ t('orders_count', ':total orders', ':total طلب', { total }) }}
        </p>
      </div>
    </div>
  </main>
...
const crumbs = computed(() => [
  ...
  { label: t('orders_title', 'My orders', 'طلباتي') },
])
...
const { items, lastPage, total, pending, error, refresh } = useOrders({ page: currentPage, perPage: PER_PAGE })
