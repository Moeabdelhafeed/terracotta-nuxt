export const useAuthConfig = () => {
  ...
  const identifiers = computed(() =>
    cfg.value.identifiers?.length ? cfg.value.identifiers : ["email"],
  );
  ...
  const socialProviders = computed(() => asList(cfg.value.social_providers));
  ...
  const allowedEmailDomains = computed(
    () => cfg.value.allowed_email_domains ?? "all",
  );
  const allowedPhoneCountries = computed(
    () => cfg.value.allowed_phone_countries ?? "all",
  );
  const isEmailDomainAllowed = (email) => {
    ...
    const domain = String(email ?? "")
      ...
      ?.toLowerCase();
    return (
      !!domain && list.map((d) => String(d).toLowerCase()).includes(domain)
    );
  };
  ...
  const inputTypeFor = (k) =>
    ({ email: "email", phone: "tel", username: "text" })[k] ?? "text";
  ...
  const identifierLabel = computed(() => {
    ...
    let base =
      parts.length > 1
        ? parts.join(` ${orWord} `)
        : (parts[0] ?? labelFor("email"));
    if (hasUsername.value && !isOtpMode.value)
      base += ` ${orWord} ${labelFor("username")}`;
    return base;
  });

  const identifierInputType = computed(() => {
    if (hasUsername.value && !isOtpMode.value) return "text";
    if (identifiers.value.length === 1)
      return inputTypeFor(identifiers.value[0]);
    return "text";
  });
  ...
  const identifierPlaceholder = computed(() => {
    ...
    const samples = identifiers.value
      .map((k) => placeholderFor(k))
      .filter(Boolean);
