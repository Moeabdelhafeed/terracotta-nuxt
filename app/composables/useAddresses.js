...
export const isWithinSaudi = (lat, lng) => {
  ...
  return (
    Number.isFinite(la) &&
    Number.isFinite(ln) &&
    la >= SAUDI_BOUNDS.minLat &&
    la <= SAUDI_BOUNDS.maxLat &&
    ln >= SAUDI_BOUNDS.minLng &&
    ln <= SAUDI_BOUNDS.maxLng
  );
};
...
export const validateAddress = (form, t) => {
  ...
  req(
    "street",
    t("address_street_required", "Street is required.", "اسم الشارع مطلوب."),
  );
  req(
    "district",
    t("address_district_required", "District is required.", "الحي مطلوب."),
  );
  req(
    "postal_code",
    t(
      "address_postal_required",
  ...
  req(
    "phone",
    t("address_phone_required", "Phone is required.", "رقم الهاتف مطلوب."),
  );

  if (
    form.building_number &&
    !ADDRESS_RULES.building_number.test(form.building_number)
  ) {
    errors.building_number = [
      t(
        "address_building_format",
        "Building number must be exactly 4 digits.",
        "رقم المبنى يجب أن يكون 4 أرقام.",
      ),
    ];
  }
  ...
  if (
    ...
    !ADDRESS_RULES.additional_number.test(form.additional_number)
  ) {
    errors.additional_number = [
      t(
        "address_additional_format",
        "Additional number must be exactly 4 digits.",
        "الرقم الإضافي يجب أن يكون 4 أرقام.",
      ),
    ];
  }
  if (
    form.short_address &&
    !ADDRESS_RULES.short_address.test(form.short_address)
  ) {
    errors.short_address = [
      t(
        "address_short_format",
        "Short address looks like RRRD2929 — 4 letters then 4 digits.",
        "العنوان المختصر بصيغة RRRD2929 — 4 أحرف ثم 4 أرقام.",
      ),
    ];
  }
  if (
    ...
    errors.lat = [
      t(
        "address_outside_saudi",
        "This address must be inside Saudi Arabia.",
        "يجب أن يكون العنوان داخل المملكة العربية السعودية.",
      ),
    ];
  }
  return errors;
...
export const useAddresses = () => {
  ...
  const addresses = computed(() => asList(data.value));
  ...
  const save = async (form, id = null) => {
    const body = { ...form };
    if (body.short_address)
      body.short_address = body.short_address.toUpperCase();
  ...
  const lookup = async (shortAddress) => {
    try {
      const res = await api("/api/addresses/lookup", {
        method: "POST",
        body: { short_address: shortAddress },
      });
      return res?.data ?? null;
    } catch (err) {
