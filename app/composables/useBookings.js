...
export const ACTIVE_STATUSES = ['pending_payment', 'confirmed', 'attending', 'preparing']
...
export const dateRange = (from, days) => {
  ...
  return Array.from({ length: days }, (_, i) => new Date(Date.UTC(y, m - 1, d + i)).toISOString().slice(0, 10))
}
...
export const dateParts = (ymd, code = 'en') => {
  ...
  const part = (options) => new Intl.DateTimeFormat(locale, { timeZone: 'UTC', ...options }).format(date)
  return { weekday: part({ weekday: 'long' }), day: part({ day: 'numeric' }), month: part({ month: 'long' }) }
}
...
export const formatSlotTime = (start, end) => [start, end].filter(Boolean).join(' – ')
...
export const catalogueBounds = (workshop, peopleCount) => {
  ...
  const max = workshop?.max_products_per_person ? people * workshop.max_products_per_person : Infinity
  return { min, max }
}

export const isCatalogueType = (type) => type === 'paint_your_piece' || type === 'make_your_candle'
...
export const bookingState = (booking) => {
  ...
  if (booking.delivery_status) return booking.delivery_status === 'completed' ? 'delivered' : booking.delivery_status
...
export const productsQuery = (products = []) =>
  Object.fromEntries(products.flatMap((line, i) => {
    const entries = []
    if (line.workshop_product_id) entries.push([`products[${i}][workshop_product_id]`, line.workshop_product_id])
    if (line.workshop_booking_piece_id) entries.push([`products[${i}][workshop_booking_piece_id]`, line.workshop_booking_piece_id])
    if (line.quantity) entries.push([`products[${i}][quantity]`, line.quantity])
    return entries
  }))

/** Only the keys the API accepts — the picker also carries titles and prices for the UI. */
export const productsBody = (products = []) =>
  products.map((line) => (line.workshop_booking_piece_id
    ? { workshop_booking_piece_id: line.workshop_booking_piece_id }
    : { workshop_product_id: line.workshop_product_id, quantity: line.quantity }))
...
export const useWorkshopBooking = (workshopId) => {
  ...
  return {
    calendar: (params) => api(`/api/workshops/${id()}/availability`, { query: params })
      .then((res) => res?.data ?? { max_available_seats: 0, blocked_dates: [] }),
    slots: (date, peopleCount) => api(`/api/workshops/${id()}/availability`, { query: { date, people_count: peopleCount } })
      .then((res) => res?.data ?? []),
    quote: ({ products, ...params }) => api(`/api/workshops/${id()}/price`, { query: { ...params, ...productsQuery(products) } }),
    create: (body) => api(`/api/workshops/${id()}/bookings`, { method: 'POST', body }),
  }
}
...
export const useBookingActions = (bookingId) => {
  ...
  return {
    ...
    removeImage: (imageId) => api(`${base()}/images/${imageId}`, { method: 'DELETE' }),
    removePiece: (pieceId) => api(`${base()}/pieces/${pieceId}`, { method: 'DELETE' }),
    deliveryQuote: (params) => api(`${base()}/delivery/quote`, { query: params }),
    chooseDelivery: (body) => api(`${base()}/delivery`, { method: 'POST', body }),
  }
}

/** `GET /api/workshops/bookings/{id}` — own bookings only; a 404 is "gone", never "forbidden". */
export const useBooking = (id) => {
  const { data, pending, error, status, refresh } = useApiFetch(() => `/api/workshops/bookings/${toValue(id)}`, {
  ...
  return {
    ...
    set: (booking) => { data.value = booking },
  }
}
...
export const useActiveBookingsCount = () => {
  ...
  const { data } = useApiFetch('/api/workshops/bookings', {
    ...
    query: { per_page: 'all' },
    transform: (res) => unwrapList(res?.data).items.filter(isActiveBooking).length,
    default: () => 0,
