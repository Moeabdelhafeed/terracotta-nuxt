...
const markHydrated = () => {
  ...
  else nuxtApp.hooks.hookOnce('app:suspense:resolve', () => { hydrated.value = true })
}

const cartEntries = computed(() => (hydrated.value ? localCartIds.value : []))
const favoriteEntries = computed(() => (hydrated.value ? localFavoriteIds.value : []))

const products = ref({})
const pending = ref(false)
const error = ref(null)

const storedIds = () => [...new Set([...localCartIds.value.map((entry) => entry.id), ...localFavoriteIds.value])]
...
const prune = (id) => {
  ...
  localFavoriteIds.value = localFavoriteIds.value.filter((favorite) => favorite !== id)
}

const hydrate = async (api, ids) => {
  if (!ids.length) return
  pending.value = true
  const results = await Promise.allSettled(ids.map((id) => api(`/api/shop/products/${id}`)))
  const fetched = {}
...
const watchStoredIds = (api) => {
  ...
  effectScope(true).run(() => {
    watch(
      () => storedIds().join(','),
      () => hydrate(api, storedIds().filter((id) => !products.value[id])),
      { immediate: true },
    )
  })
}
...
export const useLocalCart = () => {
  ...
  const items = computed(() => cartEntries.value.flatMap((entry) => {
    ...
    return [{
      ...
      in_stock: product.stock == null ? product.in_stock !== false : product.stock >= entry.quantity,
      available_stock: product.stock ?? null,
    }]
  }))

  const total = computed(() => fromHalalas(items.value.reduce((sum, line) => sum + toHalalas(line.line_total), 0)))
  const count = computed(() => cartEntries.value.reduce((sum, entry) => sum + (entry.quantity ?? 0), 0))
  ...
  const maxFor = (productId) => Math.min(products.value[productId]?.max_quantity ?? HARD_MAX_QUANTITY, HARD_MAX_QUANTITY)
  ...
  const write = (productId, quantity) => {
    ...
    localCartIds.value = existing
      ? localCartIds.value.map((entry) => (entry.id === productId ? { ...entry, quantity: capped } : entry))
  ...
  const add = async (productId, quantity = 1) => {
    ...
    return { success: true, message: t('added_to_cart', 'Added to your cart.', 'تمت الإضافة إلى عربيتك.'), errors: null, data: { id: productId, quantity: next } }
  }

  const update = async (productId, quantity) => {
    const next = write(productId, quantity)
    return { success: true, message: t('cart_updated', 'Cart updated.', 'تم تحديث العربية.'), errors: null, data: { id: productId, quantity: next } }
  }

  const remove = async (productId) => {
    localCartIds.value = localCartIds.value.filter((entry) => entry.id !== productId)
    return { success: true, message: t('cart_line_removed', 'Removed from your cart.', 'تمت الإزالة من عربيتك.'), errors: null, data: null }
  }

  return { items, total, count, hasOutOfStock, canCheckout, pending, error, refresh: () => refreshProducts(api), add, update, remove }
}
...
export const useLocalFavorites = () => {
  ...
  const favorites = computed(() => favoriteEntries.value.map((id) => products.value[id]).filter(Boolean))
  ...
  return { favorites, isFavorited, toggle, pending, error, refresh: () => refreshProducts(api) }
}
...
export const pushLocalShopToServer = async (api) => {
  ...
  if (!lines.length && !favorites.length) return { pushedLines: 0, pushedFavorites: 0, dropped: 0 }
  ...
  for (const line of lines) {
    if (await attempt(() => api('/api/shop/cart', { method: 'POST', body: { shop_product_id: line.id, quantity: line.quantity } }))) pushedLines += 1
  }
  for (const id of favorites) {
    if (await attempt(() => api(`/api/shop/favorites/${id}`, { method: 'POST' }))) pushedFavorites += 1
  }
