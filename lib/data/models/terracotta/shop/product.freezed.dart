...
/// and the identifier the cart and favourites endpoints take.
 int get id;/// Already-localized display name for the requested locale.
 String get title;/// List price as a decimal string (`"45.00"`). Never a double.
 String get price;/// Discounted price as a decimal string (`"35.00"`), or null when
/// the product is not on offer. Non-null is what makes it an offer.
 String? get salePrice;/// Card thumbnail. NULL for CMS rows whose image has not been set —
/// render through `image?.display` with a placeholder fallback.
 ApiImage? get image;/// CMS-curated flag driving the home `featured_products` rail.
/// Independent of [salePrice] — a product can be featured, on
/// offer, both, or neither.
 bool get isFeatured;/// Per-user favourite state at the time this list was fetched.
 bool get isFavorited;/// Whether the studio has any of this to sell. False is a piece the
...
@override
String toString() {
  return 'Product(id: $id, title: $title, price: $price, salePrice: $salePrice, image: $image, isFeatured: $isFeatured, isFavorited: $isFavorited, inStock: $inStock, stock: $stock, maxQuantity: $maxQuantity)';
}
...
class _Product extends Product {
  const _Product({required this.id, required this.title, required this.price, this.salePrice, this.image, required this.isFeatured, required this.isFavorited, required this.inStock, this.stock, required this.maxQuantity}): super._();
