...
class GlobalFilledButton extends StatefulWidget {
  ...
  /// If null, uses a green background with a white checkmark.
  final ButtonStateStyle? successStyle;

  /// Style override for the error completion state.
  /// If null, uses a red background with a white X icon.
  final ButtonStateStyle? errorStyle;

  /// Tonal emphasis — the M3 step between filled and outlined: a soft
  /// role-aware primary tint with a primary label, for the secondary
  /// action sitting next to a filled primary one. Explicit
  /// `style.backgroundColor` / `foregroundColor` win.
  final bool tonal;
  ...
  @override
  State<GlobalFilledButton> createState() => _GlobalFilledButtonState();
}

class _GlobalFilledButtonState extends State<GlobalFilledButton>
    with ButtonBehaviorMixin<GlobalFilledButton> {
  ...
  @override
  void didUpdateWidget(GlobalFilledButton oldWidget) {
  ...
  ResolvedColors _resolveColors(
    ...
    // Tonal defaults: an OPAQUE soft primary tint (alpha-blended over
    // the surface so the contrast audit still applies) + primary label.
    final primary = context.primaryColors.primary;
    final tonalBg = widget.tonal
        ? Color.alphaBlend(
    ...
    final bg =
        ...
        tonalBg ??
        context.buttonsColors.primary;
    final fg =
        stateStyle?.foregroundColor ??
        baseStyle?.foregroundColor ??
        (widget.tonal
            // High-contrast primary: on a soft tint, the raw primary
            // (e.g. guest pink) measured 2.4:1 — unreadable.
            ? context.primaryColors.primaryHighContrast
            : context.textColors.onPrimary);

    final disabledBgGradient = _themedDisabledStyle?.backgroundGradient;
    final disabledBg =
        _themedDisabledStyle?.backgroundColor ?? context.buttonsColors.disabled;
    final disabledFg =
  ...
  Widget build(BuildContext context) {
    ...
    _btnTheme = btnTheme;
    final themeBase = btnTheme?.filledStyle;
    ...
    final content = AnimatedButtonContent(
      ...
      slotsAtEdges: _base?.slotsAtEdges ?? false,
      slotEdgeInset: _base?.slotEdgeInset ?? kSlotSpacing,
    );
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    // The SAME layout flags as the resting content. The
                    // shimmer overlay redraws the label and slots on top
                    // of itself, and without these it fell back to the
                    // centred row — so a pinned arrow jumped inward the
                    // moment the button started loading, and back out
                    // when it finished.
                    //
                    // `Center` centres a min-width row; an edge layout
                    // has to FILL, or there are no edges to pin to.
                    child: _base?.slotsAtEdges ?? false
                        ? SizedBox.expand(
                            child: AnimatedButtonContent(
                              ...
                              slotsAtEdges: true,
                              slotEdgeInset:
                                  _base?.slotEdgeInset ?? kSlotSpacing,
                            ),
                          )
                        : Center(
                            child: AnimatedButtonContent(
      ...
      case LoadingType.border:
        return KeyedSubtree(
          ...
          child: BorderLoadingOverlay(
            // Foreground, not background — the trace paints ON the
            // button's own surface, so a background-colored stroke is
            // invisible (outlined/text already use foreground).
