...
class GlobalTextButton extends StatefulWidget {
  const GlobalTextButton({
    ...
    this.link = false,
    this.underline = LinkUnderline.always,
  ...
  /// Render as a web-style LINK instead of a padded button: the box
  /// hugs the label (no horizontal padding, no preset height) and the
  /// underline in [underline] applies.
  ///
  /// The 48dp hit area is UNCHANGED — `MinTouchTarget` expands the
  /// touch region without painting, so a link stays as easy to hit as
  /// a button. Use this for tertiary/footnote actions ("Use a different
  /// email", "Learn more"); keep the padded default for dialog and
  /// sheet action rows, where the box is the expected affordance.
  final bool link;

  /// Underline behaviour in [link] mode (ignored otherwise).
  ///
  /// Defaults to [LinkUnderline.always]: colour alone as the only
  /// affordance fails WCAG 1.4.1 wherever a link sits in running text.
  /// [LinkUnderline.onHover] is fine for a standalone action that is
  /// visually obvious on its own.
  ...
  /// If null, uses a primary-container background with a checkmark.
  final ButtonStateStyle? successStyle;

  /// Style override for the error completion state.
  /// If null, uses an error-container background with an X icon.
  ...
  @override
  State<GlobalTextButton> createState() => _GlobalTextButtonState();
}

class _GlobalTextButtonState extends State<GlobalTextButton>
    with ButtonBehaviorMixin<GlobalTextButton> {
  // Mirrored from the surface purely to drive the link underline. Only
  // link mode rebuilds on them — a padded button's hover/focus visuals
  // are already handled inside ButtonSurface.
  bool _hovered = false;
  bool _focused = false;

  void _handleHover(bool value) {
    if (widget.link && _hovered != value) setState(() => _hovered = value);
    widget.onHover?.call(value);
  }

  /// Reported by the surface: focused AND not focused by a pointer.
  /// The underline follows keyboard focus only — a tap should not leave
  /// it ruled on.
  void _handleFocusVisibility(bool value) {
    if (widget.link && _focused != value) setState(() => _focused = value);
  }
  ...
  @override
  void didUpdateWidget(GlobalTextButton oldWidget) {
  ...
  ResolvedColors _resolveColors(
    ...
    // Text button: background is always transparent
    final fg =
        stateStyle?.foregroundColor ??
        _base?.foregroundColor ??
  ...
  ResolvedSize _resolveSize(
    ...
    final finalWidth =
        wOverride ??
        ((widget.shrinkWidth || widget.link) ? null : double.infinity);

    // Link mode: never synthesise a box height. An explicit
    // style.height still wins if a caller really wants one.
    if (widget.link) {
      return ResolvedSize(width: finalWidth, height: hOverride);
    }
  ...
  ResolvedDecoration _resolveDecoration(ButtonStateStyle? stateStyle) {
    ...
    // A link has no box of its own: the label IS the target.
    final defaultPadding = widget.link
        ? EdgeInsets.zero
        : EdgeInsets.symmetric(
            horizontal:
                widget.size?.horizontalPadding ?? kDefaultHorizontalPadding,
          );
    ...
    // Text button: always 0 elevation, no border
  ...
  Widget build(BuildContext context) {
    ...
    _btnTheme = btnTheme;
    final themeBase = btnTheme?.textStyle;
    ...
    Widget content = AnimatedButtonContent(
      slots: slots,
      style: resolvedStyle,
    ...
    if (widget.link) {
      content = LinkUnderlineDecorator(
        mode: widget.underline,
        // Hover and focus both extend the rule: a keyboard user should
        // get the same affordance a pointer user does.
        active: _hovered || _focused,
        color: resolvedStyle.textStyle.color ?? colors.foreground,
        duration: _morph,
    ...
    final baseButton = ButtonSurface(
      ...
      onFocusVisibilityChanged: _handleFocusVisibility,
      focusNode: widget.focusNode,
      autofocus: widget.autofocus,
      padding: decoration.padding,
      // A link shows focus by ruling its underline in; a ring on top
      // of that is both redundant and cramped against the glyphs.
      showFocusRing: !widget.link,
      morphDuration: _morph,
      morphCurve: widget.morphCurve,
    );
  ...
  Widget? _buildLoadingOverlay({
    ...
    switch (_effectiveLoadingType) {
      case LoadingType.shimmer:
        ...
        final baseColor =
            _themedLoadingStyle?.shimmerBaseColor ?? fg.withValues(alpha: 0.15);
